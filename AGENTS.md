...
│   ├── use-nav.ts         # Navigation item/group hooks (passthrough, no auth wired up)
...
Each item also supports an optional `access` property (`requireOrg`, `permission`, `role`, `plan`, `feature`) defined in `src/types/index.ts`, but `useFilteredNavItems()` / `useFilteredNavGroups()` in `src/hooks/use-nav.ts` currently pass items through unfiltered — there's no auth provider wired up to evaluate those checks. Add real filtering logic there if/when an auth system is introduced.
...
node scripts/cleanup.js notifications sentry

# Preview without changing files
node scripts/cleanup.js --dry-run sentry
